...
**8.2c ✓ CODÉ — rendu 3D à deux tiers (focus détaillé / conscience imposteur LOD) + `tools/foule-3d.sh`.**
⏳ **EN ATTENTE DE TA VÉRIF À L'ÉCRAN** (je n'ai pas de GPU) : `./tools/foule-3d.sh 80` → confirmer qu'on
voit > 64 silhouettes, ~8 détaillées + la foule en imposteurs, sans chute de FPS. D24 ne sera coché FERMÉ
qu'après ça.

**PROCHAINE ACTION CONCRÈTE (après ta vérif 8.2c) = 8.3** (cellules spatiales + hôte agrégateur : 1 flux
résumé par région → tenir l'invariant jusqu'à 5000 ; corrige aussi le tri focus O(N log N) via index spatial),
OU bifurquer sur la Phase B (inclusivité : D3/D4/D5). Voir §D, Chapitre 8.
...
> - **D24 (🟠, ch.8.2c) — foule visible plafonnée à 64** : code FAIT (rendu à deux tiers, détaillés ≤ 64 +
>   imposteurs LOD ≤ 512) ⏳ **à CONFIRMER à l'écran par l'utilisateur** (`tools/foule-3d.sh 80`) — pas
>   coché fermé tant que ce n'est pas vu en vrai.
  ...
  **+ Amorce D24 :** relier `MAX_AVATARS` au tier de rendu — focus en avatars détaillés (borne
  ...
  - **8.2c ✓ CODÉ (19 juin) — rendu 3D à deux tiers (ferme l'amorce D24) — ⏳ VÉRIF VISUELLE = utilisateur.**
    ...
    **51 tests, 0 warning, build vert.** ⚠️ *JE NE PEUX PAS LE VOIR (pas de GPU ici) — c'est TOI qui valides :*
    ...
    dis-le, on ajustera.* **→ D24 amorcé ; il ne sera coché FERMÉ qu'après TA confirmation à l'écran.**
