# La documentation — par où vous voulez

> **Bienvenue.** Cette doc se lit comme une revue : chaque page est une étape, et **où que vous commenciez, vous
> êtes au bon endroit**. Vous pouvez suivre un parcours de bout en bout, sauter à ce qui vous intéresse, ou revenir
> ici à tout moment. Aucun prérequis : les termes techniques sont expliqués au passage, et regroupés dans le
> [📖 glossaire](glossaire.md).
>
> Vous cherchez la présentation générale du projet ? C'est la [🗺️ vitrine (README)](../README.md).

---

## Choisissez votre parcours

**🌱 Découvrir — l'idée, sans jargon**
1. [Le « sans serveur », en clair](comprendre-le-p2p.md) — ce qu'on enlève, et par quoi on le remplace
2. [L'état du projet, chiffré](etat-du-projet.md) — ce qui marche, mesuré
3. [Le registre des doutes](doutes.md) — les questions encore ouvertes

**🔎 Juger vite — ce qui est prouvé, ce qui ne l'est pas**
1. [L'état du projet, chiffré](etat-du-projet.md) — les mesures et leurs limites
2. [Le registre des doutes](doutes.md) — la frontière honnête, assumée
3. [Les coulisses](coulisses.md) — les enquêtes : comment une mesure a, plusieurs fois, corrigé le plan

**⚙️ Le code & l'architecture — pour qui veut le détail technique**
1. [Architecture & organisation du code](ARCHITECTURE.md) — les modules, le format d'un paquet, la cible long terme
2. [Chantier réseau](chantier-reseau.md) — confronter au réel, caractériser chaque lien, durcir la confiance
3. [Chantier foule dense](chantier-foule.md) — percevoir une foule sans plafond ni explosion de débit
4. [Chantier robustesse](chantier-robustesse.md) — autorité, longévité, traversée des NAT difficiles
5. [Comment lancer & tester](TESTS.md) — rejouer les mesures vous-même

**🧭 Tout comprendre, dans l'ordre — du plus accessible au plus pointu**
1. [Le « sans serveur », en clair](comprendre-le-p2p.md)
2. [L'état du projet, chiffré](etat-du-projet.md)
3. [Le registre des doutes](doutes.md)
4. [Les coulisses](coulisses.md)
5. [Le journal de développement](journal.md)
6. [Architecture & organisation du code](ARCHITECTURE.md)
7. [Chantier réseau](chantier-reseau.md)
8. [Chantier foule dense](chantier-foule.md)
9. [Chantier robustesse](chantier-robustesse.md)
10. [Comment lancer & tester](TESTS.md)

---

## Toutes les pages, en un coup d'œil

| Page | En une phrase |
|---|---|
| [Le « sans serveur », en clair](comprendre-le-p2p.md) | L'idée du projet en mots simples, problème par problème. |
| [L'état du projet, chiffré](etat-du-projet.md) | La revue honnête : ce qui est mesuré, et ce que ça ne prouve pas. |
| [Le registre des doutes](doutes.md) | Les questions ouvertes, suivies — un doute n'est pas caché, c'est l'objet du travail. |
| [Les coulisses](coulisses.md) | Le carnet d'enquêtes : les murs, les fausses pistes, et ce qu'on en a appris. |
| [Le journal de développement](journal.md) | L'histoire du cœur, chapitre par chapitre. |
| [Architecture & code](ARCHITECTURE.md) | L'organisation des fichiers, le format d'un paquet, l'architecture cible. |
| [Chantier réseau](chantier-reseau.md) | Réseau réel, sonde de lien, redondance adaptative, confiance, identité. |
| [Chantier foule dense](chantier-foule.md) | Voir la foule entière sans plafond, à débit borné. |
| [Chantier robustesse](chantier-robustesse.md) | Autorité par objet, longévité, relais pour les NAT difficiles. |
| [Comment lancer & tester](TESTS.md) | Toutes les commandes pour rejouer les mesures soi-même. |
| [📖 Glossaire](glossaire.md) | Chaque terme technique, expliqué en une phrase. |

---

*🗺️ [Revenir à la vitrine du projet](../README.md) · 📖 [Glossaire](glossaire.md)*
